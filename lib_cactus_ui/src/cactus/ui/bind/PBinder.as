package cactus.ui.bind
{
	...
	public class PBinder
	{
		 ...
		 * @param resPosY
		 */
		private static function layoutPosition(view:PView, resource:MovieClip, childname:String, childnameWithoutPostfix:String, resPosX:Number, resPosY:Number,resWidth:Number,resHeight:Number, mt:MTObj=null):void
		{
			// 被操作对象
			var item:DisplayObject=view[childname];
			
			//			// 如果是button，还没有加载到显示列表
			//			if (resWidth == 0 && resHeight == 0)
			//			{
			//				resWidth=resource[childnameWithoutPostfix].width;
			//				resHeight=resource[childnameWithoutPostfix].height;
			//			}
			
			// 自适应对齐的中心点
			var mx:Number=item.x + resWidth / 2;
			var my:Number=item.y + resHeight / 2;
			
			// 距离四方百分比
			var leftPercent:Number=resPosX / Global.originalScreenW;
			var rightPercent:Number=1 - (resPosX + resWidth) / Global.originalScreenW;
			var topPercent:Number=resPosY / Global.originalScreenH;
			var bottomPercent:Number=1 - (resPosY + resHeight) / Global.originalScreenH;
			...
			if ((layout & MASK_R) == MASK_R)
				item.x=Global.screenW - (rightPercent * Global.screenW + resWidth);
			if ((layout & MASK_B) == MASK_B)
				item.y=Global.screenH - (bottomPercent * Global.screenH + resHeight);
			
			if ((layout & MASK_ALIGN_LEFT) == MASK_ALIGN_LEFT)
				item.x=resPosX;
			if ((layout & MASK_ALIGN_CENTER) == MASK_ALIGN_CENTER)
				item.x=(Global.screenW - resWidth) / 2;
		...
		private static function addResourceToView(view:PView, resource:MovieClip, properties:Array):void
		{
			while (resource.numChildren > 0)
			{
				...
				{
					if (properties[j].propertyName == (instance.name + PUIConfig.PB) || properties[j].propertyName == instance.name)
					{
						propertyIndex=j;
						break;
					}

...
package cactus.common.tools.util
{
	...
	public class Pen
	{
		...
		public function Pen(gTarget:Graphics = null)
		{
			if (gTarget)
				_gTarget = gTarget;
		}
		 ...
		 * @param pAlpha
		 */
		public function drawSector(x:Number, y:Number, r:Number, angle:Number, startFrom:Number = 270, color:Number = 0xff0000, pAlpha:Number = 1):void
		{
			_gTarget.clear();
			_gTarget.beginFill(color, pAlpha);
			...
			*/
			...
			_gTarget.lineTo(x + r * Math.cos(startFrom), y + r * Math.sin(startFrom));
			for (var i:Number = 1; i <= n; i++)
			{
				...
				_gTarget.curveTo(bx, by, cx, cy);
			}
			if (angle != 360)
			{
				_gTarget.lineTo(x, y);
			}
			_gTarget.endFill(); // if you want a sector without filling color , please remove this line.
		}

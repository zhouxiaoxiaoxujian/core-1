package cactus.common.manager
{
	 ...
	 * 音效控制器
	 * @author Pengx
	 * @e-mail onchenxing@yahoo.com.cn
	 */
	public class SoundManager extends EventDispatcher
	{
		...
		{
			if(this._soundsDict[name] != null)	return false;
		...
		{			
			...
			try
			{
				channelObj.channel = soundObj.sound.play(channelObj.startTime,channelObj.loops,new SoundTransform(channelObj.volume));
			...
			try
			{
				if(id == -1)
				{
					...
					for(var i:int = 0 ; i < soundObj.channels.length ; i++)
					{
						...
						{
							channelObj.channel = soundObj.sound.play(channelObj.position,channelObj.loops,new SoundTransform(channelObj.volume));
							innerPlaySound(channelObj);	
						}
					}
				}
		...
		{
			if(_bgChannelObj == null) return null;

...
package cactus.common.behaviour2d
{
	...
	public class BaseBehaviour2d
	{
		 ...
		 * 行为目标，即执行行为的主角
		 */
		protected var _owner : Vehicle;

		public function BaseBehaviour2d($targetDisplayObject : Vehicle)
		{
			owner = $targetDisplayObject;
		...
		public function update(delay : int) : void
		{
			...
			owner.velocity = velocity.add(_steeringForce);
			_steeringForce = new Vector2D();
		}

		protected function get position() : Vector2D
		{
		...
		protected function set velocity(value : Vector2D) : void
		{
		...
		public function followPath(path : Array, loop : Boolean = false) : void
		{
			...
			if (wayPoint == null)
				return;
			if (position.dist(wayPoint) < _pathThreshold)
			{
		...
		public function flock(vehicles : Array) : void
		{
			...
			for (var i : int = 0; i < vehicles.length; i++)
			{
				...
				if (vehicle != owner && inSight(vehicle))
				{
		...
		{
			if (position.dist(vehicle.position) > _inSightDist)
				return false;
		...
		{
			return position.dist(vehicle.position) < _tooCloseDist;
		}
	}
}
